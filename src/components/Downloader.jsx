...
const Downloader = () => {
  ...
  useEffect(() => {
    socketRef.current = io("https://ytclipbackend.online/");
    ...
    try {
      const response =
        downloadMethod === "playlist"
          ? await axios.post(
              "https://ytclipbackend.online/api/v1/downloads/playlist",
              { url: playlistUrl }
            )
          : await axios.post(
              "https://ytclipbackend.online/api/v1/downloads/video",
              {
                url,
              }
            );
  ...
  const handleDownload = async (video, type) => {
    ...
    const downloadUrl = `https://ytclipbackend.online${base}${itag}&startTime=${startTime}&duration=${
      endTime - startTime
    }`;
